...
const StudentDetailModal = ({ student, onClose }) => {
    ...
    const processCPC = (allReports, langId, allTopics = []) => {
        const langReports = allReports.filter(r => (r.languageId?._id || r.languageId) === langId);
        ...
        if (topicsToShow.length > 0) {
            topicRows = topicsToShow.map(topic => {
                const topicId = topic._id;
                const reportsForTopic = langReports.filter(r => (r.topicId?._id || r.topicId) === topicId);
            ...
            langReports.forEach(report => {
                ...
                if (!topicGroups[topicId]) {
                    topicGroups[topicId] = {
                        topicId: topicId,
                        topicName: report.topicId?.name || 'Unknown Topic',
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-100">
                                        ...
                                        <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Dates</th>
                                        <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider text-center">Days ({totalSum})</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-50">
                                    {cpcData.length === 0 ? (
                                        <tr><td colSpan="4" className="px-4 py-10 text-center text-gray-400 italic text-sm">No CPC data for this language.</td></tr>
                                    ) : (
                                        cpcData.map((row) => (
                                            <tr key={row.no} className="hover:bg-indigo-50/30 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-xs text-gray-500 font-medium">
                                                    {row.startDate ? (
                                                        <>
                                                            {format(parseISO(row.startDate), 'dd MMM')} - {row.endDate ? format(parseISO(row.endDate), 'dd MMM yyyy') : (
                                                                row.isStarted ? (
